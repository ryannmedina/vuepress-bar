...
const getDirectories = (source, excludeDirectory) => {

  excludeDirectory.push('.vuepress');

  return readdirSync(source).filter(name => !excludeDirectory.includes(name) && isDirectory(join(source, name)));
}
...
function side(
  baseDir,
  { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, addReadMeToFirstGroup, mixDirectoriesAndFilesAlphabetically, excludeDirectory },
  relativeDir = "",
  currentLevel = 1
) {
  const fileLinks = getChildren(baseDir, relativeDir, currentLevel > maxLevel);
  if (currentLevel <= maxLevel) {
    getDirectories(join(baseDir, relativeDir), excludeDirectory)
      .filter(subDir => !subDir.startsWith(navPrefix))
      .forEach(subDir => {
        const children = side(
          baseDir,
          { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, excludeDirectory },
 ...
 */
function nav(rootDir, { navPrefix, stripNumbers, skipEmptyNavbar, excludeDirectory }, relativeDir = "/", currentNavLevel = 1) {
  relativeDir = relativeDir.replace(/\\/g, "/");
  const baseDir = join(rootDir, relativeDir);
  const childrenDirs = getDirectories(baseDir, excludeDirectory).filter(subDir => subDir.startsWith(navPrefix));
